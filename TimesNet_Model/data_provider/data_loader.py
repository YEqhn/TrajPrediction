...
class CustomSegLoader(Dataset):
    def __init__(self, args, root_path, win_size, step=1, flag="train"):
        ...
        # 指向你刚刚生成的三个 npy 文件
        train_path = os.path.join(root_path, "CustomAD_train.npy")
        test_path = os.path.join(root_path, "CustomAD_test.npy")
        label_path = os.path.join(root_path, "CustomAD_test_label.npy")

        train_data = np.load(train_path)
        test_data = np.load(test_path)
        test_label = np.load(label_path)

        # 必须在训练集上 fit StandardScaler，然后应用到测试集，保证归一化尺度一致
        self.scaler.fit(train_data)
        train_data = self.scaler.transform(train_data)
        test_data = self.scaler.transform(test_data)
